      ...
      {open && (
        ...
        >
          <motion.div
            ...
            onClick={(e) => e.stopPropagation()} // чтобы клик по самой модалке не закрывал её
          >
            ...
            {replies.length === 0 ? (
              ...
              <div className="space-y-4">
                {replies.map((reply, index) => (
                  ...
                  >
                    <p><span className="text-white/70">Имя:</span> {reply.name}</p>
                    <p><span className="text-white/70">Телефон:</span> {reply.phone}</p>
                    <p><span className="text-white/70">Тип:</span> {reply.type === "trip" ? "Поездка" : "Посылка"}</p>
                    <p><span className="text-white/70">Кол-во человек:</span> {reply.count}</p>
                    {reply.comment && (
                      <p><span className="text-white/70">Комментарий:</span> {reply.comment}</p>
                    )}
                  </div>
                ))}
              </div>
